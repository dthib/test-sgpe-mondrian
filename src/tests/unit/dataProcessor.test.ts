import { calculateSNBCData, SECTOR_COLORS, LEVER_COLORS } from '../../utils/dataProcessor';
  ...
  describe('SECTOR_COLORS', () => {
    it('should have colors for all expected sectors', () => {
      const expectedSectors = [
        'Transports', 'Résidentiel', 'Tertiaire', 'Industrie', 
        'Agriculture', 'Déchets', 'UTCATF', 'Cultures'
      ];
  ...
  describe('calculateSNBCData', () => {
    ...
    it('should calculate data for all sectors', () => {
      ...
      expect(result.sectors.map(s => s.name)).toEqual([
        'Transports', 'Résidentiel', 'Industrie', 'Agriculture', 'UTCATF'
      ]);
    });
    ...
    it('should handle missing sectors gracefully', () => {
      ...
      const result = calculateSNBCData(epciDataWithMissingSector, mockLeverPercentages);
      
      // Should still process valid sectors
      expect(result.sectors).toHaveLength(5);
      expect(result.sectors.find(s => s.name === "SecteurInexistant")).toBeUndefined();
    });
    ...
    it('should handle missing regions gracefully', () => {
      ...
      const result = calculateSNBCData(mockEpciData, leverPercentagesWithMissingRegion);
    ...
    describe('Method 1: difference_repartition', () => {
      it('should calculate correct contributions for Transports', () => {
        const result = calculateSNBCData(mockEpciData, mockLeverPercentages);
        const transportsSector = result.sectors.find(s => s.name === 'Transports');
        ...
        const vehiculesElectriques = transportsSector!.levers.find(l => l.name === 'Véhicules électriques');
        expect(vehiculesElectriques!.contribution).toBeCloseTo(expectedReduction * 0.19, 1);
        
        const fretDecarbone = transportsSector!.levers.find(l => l.name === 'Fret décarboné');
        expect(fretDecarbone!.contribution).toBeCloseTo(expectedReduction * 0.23, 1);
        
        expect(transportsSector!.total).toBeCloseTo(
          vehiculesElectriques!.contribution + fretDecarbone!.contribution, 1
        );
      });

      it('should calculate correct contributions for Résidentiel', () => {
        const result = calculateSNBCData(mockEpciData, mockLeverPercentages);
        const residentielSector = result.sectors.find(s => s.name === 'Résidentiel');
        ...
        const chaudieresFioul = residentielSector!.levers.find(l => l.name === 'Changement chaudières fioul');
        expect(chaudieresFioul!.contribution).toBeCloseTo(expectedReduction * 0.43, 1);
        
        const sobriete = residentielSector!.levers.find(l => l.name === 'Sobriété bâtiments');
    ...
    describe('Method 2: transposition_directe', () => {
      it('should calculate correct contributions for Agriculture', () => {
        const result = calculateSNBCData(mockEpciData, mockLeverPercentages);
        const agricultureSector = result.sectors.find(s => s.name === 'Agriculture');
        ...
        const batiments = agricultureSector!.levers.find(l => l.name === 'Bâtiments & Machines');
        expect(batiments!.contribution).toBeCloseTo(expectedTarget * 0.333, 1);
        
        const elevage = agricultureSector!.levers.find(l => l.name === 'Élevage durable');
        expect(elevage!.contribution).toBeCloseTo(expectedTarget * 0.333, 1);
        
        const fertilisation = agricultureSector!.levers.find(l => l.name === 'Fertilisation azotée');
        expect(fertilisation!.contribution).toBeCloseTo(expectedTarget * 0.334, 1);
    ...
    describe('Method 3: sum_transposition', () => {
      it('should calculate correct contributions for Industrie', () => {
        const result = calculateSNBCData(mockEpciData, mockLeverPercentages);
        const industrieSector = result.sectors.find(s => s.name === 'Industrie');
        ...
        const production = industrieSector!.levers.find(l => l.name === 'Production industrielle');
      ...
      it('should handle missing source_sectors', () => {
        const epciDataWithoutSourceSectors = {
          ...
          sectors: mockEpciData.sectors.map(s => 
            s.name === 'Industrie' 
              ? { ...s, source_sectors: undefined }
              : s
          )
        };

        const result = calculateSNBCData(epciDataWithoutSourceSectors, mockLeverPercentages);
        const industrieSector = result.sectors.find(s => s.name === 'Industrie');
    ...
    describe('Method 4: national_repartition', () => {
      it('should calculate correct contributions for UTCATF', () => {
        ...
        const pratiques = utcatfSector!.levers.find(l => l.name === 'Pratiques stockantes');
        expect(pratiques!.contribution).toBeCloseTo(expectedChange * 0.34, 1);
        
        const haies = utcatfSector!.levers.find(l => l.name === 'Gestion des haies');
    ...
    describe('Error handling', () => {
      it('should handle unknown calculation method', () => {
        ...
        const result = calculateSNBCData(epciDataWithUnknownMethod, mockLeverPercentages);
        
        // Should skip sectors with unknown methods
        expect(result.sectors.find(s => s.name === 'Transports')).toBeUndefined();
      });
