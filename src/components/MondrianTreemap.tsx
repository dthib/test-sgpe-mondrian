...
export const MondrianTreemap: React.FC<MondrianProps> = ({
  ...
  const [currentView, setCurrentView] = useState<'overview' | 'sector' | 'lever'>('overview');
  ...
  const buildTreeMapData = useCallback(() => {
    ...
    return calculatedData.sectors.map(sector => ({
      ...
      children: sector.levers.map(lever => ({
        ...
        itemStyle: { color: lever.color || sectorColors[sector.name] || defaultColor }
      }))
    }));
  ...
  useEffect(() => {
    ...
    const sectorData = calculatedData.sectors.find(s => s.name === selectedSector);
  ...
  const handleChartClick = useCallback(
    (params: any) => {
      if (params?.data?.sector && params?.data?.lever) {
        ...
        const sectorData = calculatedData.sectors.find(s => s.name === sector);
        if (!sectorData) return;
  ...
  const buildChartOptions = useCallback(() => {
    ...
    if (treeData.length === 1 && treeData[0].children && treeData[0].children.some((c: any) => c.selected)) {
      styledTreeData = [{ ...treeData[0], children: applyCustomItemStyle(treeData[0].children) }];
    }
      ...
      tooltip: {
        formatter: function (info: any) {
          ...
          if (data.sector && data.lever) {
            ...
            const target = data.realValue !== undefined ? data.realValue : data.contribution;
      ...
      series: [{
        ...
        label: {
          ...
          formatter: function (params: any) {
            ...
            if (data.sector && data.lever) {
              const val = (data.realValue !== undefined ? data.realValue : data.contribution) as number;
              return `${data.lever}\n\n${val.toFixed(1)} ktCO₂e`;
            }
            const val = (data.realValue !== undefined ? data.realValue : data.value) as number;
            return `${data.name}\n\n${val.toFixed(1)} ktCO₂e`;
          },
          fontSize: 12,
          color: '#111'
    ...
    return (
      <div className="mondrian-error">
        ...
        <p className="mondrian-error-hint">
          Please verify the mapping between EPCI sectors and lever percentages.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`mondrian-treemap mondrian-treemap-container ${className}`} style={style}>
      {/* Custom Breadcrumb */}
      {(currentView === 'sector' || selectedSector) && (
        <div className="mondrian-breadcrumb">
          ...
          {selectedLever && (
            <>
              <span className="mondrian-breadcrumb-separator">›</span>
              <span className="mondrian-breadcrumb-item">{selectedLever}</span>
            </>
          )}
        </div>
      )}
