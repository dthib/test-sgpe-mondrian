    ...
    for (const sectorEPCI of epciData.sectors) {
        ...
        switch (sectorEPCI.calculation_method) {
            case "difference_repartition":
                total = calculateDifferenceRepartition(
                    sectorEPCI,
                    sectorCSV,
                    epciData.region,
                    levers
            ...
            case "transposition_directe":
                total = calculateTranspositionDirecte(
                    sectorEPCI,
                    sectorCSV,
                    epciData.region,
                    levers
            ...
            case "sum_transposition":
                total = calculateTranspositionSum(
                    sectorEPCI,
                    sectorCSV,
                    epciData,
                    levers
            ...
            default:
                console.error(
                    `Unknown calculation method: ${sectorEPCI.calculation_method}`
                );
        }
...
function calculateTranspositionSum(
    ...
    for (const lever of sectorCSV.levers) {
        ...
        if (percentage === undefined) {
            console.warn(
                `Region "${epciData.region}" not found for lever "${lever.name}"`
            );
            continue;
        }
